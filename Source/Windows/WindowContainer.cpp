...
public:
    ...
    void mouseDrag (const juce::MouseEvent& mouseEvent)
    {
        ...
        if ((offset < 0 && (offset < -(parent.isVertical ? prevComponent.getHeight() : prevComponent.getWidth()) + minimumComponentSize)) || (offset > 0 && (offset > (parent.isVertical ? nextComponent.getHeight() : nextComponent.getWidth()) - minimumComponentSize)))
        {
            return;
        }
...
WindowContainer::WindowContainer (MainComponent& mainComponent_, juce::ValueTree layoutTree_)
...
void WindowContainer::componentAddedOrRemoved()
{
    ...
    resizeBars.resize (std::max<size_t> (1, childComponents.size()) - 1);
...
void WindowContainer::valueTreeChildAdded (juce::ValueTree& parentTree, juce::ValueTree& childWhichHasBeenAdded)
{
    ...
    {
        componentToAdd = std::make_shared<WindowContainer> (mainComponent, childWhichHasBeenAdded);
    }
    else if (! (componentToAdd = mainComponent.getOrCreateWindow (childWhichHasBeenAdded)))
    {
        return;
    }

...
class Mel : public OscSender
{
public:
    void initialise()
    {
        mel = essentia::standard::AlgorithmFactory::create ("MelBands", "inputSize", size / 2 + 1);
        mel->input ("spectrum").set (input);
        mel->output ("bands").set (output);
        spectrum.initialise();
    }

    void setSampleRate (const float sampleRate)
    {
        mel->configure ("sampleRate", sampleRate);
    }
